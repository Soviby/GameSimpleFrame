...
public class ReferenceCollectorEditor : Editor
{
    static Type[] sAllComponentTypes =
    {
        ...
        typeof(MyGridLayoutGroup),
        typeof(MyHorizontalLayoutGroup),
        typeof(MyVerticalLayoutGroup),
        typeof(MyImage),
        typeof(MyText),
    ...
    {
        string path;
        if (SyncUICodeInDir(rc, "/Scripts/Game/", out path, genCacheByRC))//读取的根路径
        {
    ...
    {
        int line = 0;
        if (FindUICodeInDir(rc, "/Scripts/Game/", ref path, out line))
        {
