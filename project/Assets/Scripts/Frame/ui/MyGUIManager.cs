...
public class MyGUIManager : Singleton<MyGUIManager>, IUpdateable
{
    ...
    Dictionary<string, UIConfig> itemCofigMap = new Dictionary<string, UIConfig>()
    {
        ...
        new UIConfig()
        {
            className = "PreparePlayerItem",
            resName = "SelectPlayerItem",
        },
        ["PlayerInfoItem"] =
        new UIConfig()
        {
        ...
        ["BattlePlayerRawImage"] =
        new UIConfig()
        {
            className = "BattlePlayerRawImage",
        },
        ["BattlePlayerP1Item"] =
        new UIConfig()
        {
            className = "BattlePlayerP1Item",
        },
        ["BattlePlayerOtherItem"] =
        new UIConfig()
        {
            className = "BattlePlayerOtherItem",
        },
    };
    ...
    public void Close<T>() where T : MyPanel, new()
    {
        var panel = GetExistPanel<T>();
        panel?.Close();
    }
